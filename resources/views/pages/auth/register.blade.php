@php
    $teams = App\Models\Team::where('name', '!=', 'Master Admin Team')->pluck('name', 'id');
@endphp
...
<x-layouts::auth :title="__('Register')">
    <div class="flex flex-col gap-6">
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
            ...
            <!-- Team/Clinic -->
            <flux:select name="team_id" :label="__('Team')" :placeholder="__('Select a team')" required>
                @foreach ($teams as $id => $name)
                    <flux:select.option value="{{ $id }}" :selected="old('team_id') == $id">
